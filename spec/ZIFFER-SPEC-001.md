...
>
> **⚠ CORRECTION (v1.3.32). A validator, not a clause, was telling operators to invent a role name.** `attestation_object.schema.json` declared `minItems: 1` on `required_roles` and **AT-1 stated no lower bound**, so the two disagreed about the common case rather than an edge one: the business simulation builds a floor-HIGH Attestation Object with `required_roles: []` for four of its five floor-HIGH actions, because its policy names a role for one task and demands a quorum without naming one for the rest. Every one of those objects was schema-invalid while both verifiers accepted it, and the only way to satisfy the schema was to invent a role name for a requirement the rule does not have. **The bound is removed and AT-1 now says what an empty list means**: the rule demanded a quorum and named no role, so any enrolled attester of the right kind satisfies it. It withholds nothing a verifier needs — the threshold is `quorum_k`, recomputed from the signed bundle and never read from the object (AT-9, PB-6), and role membership is resolved against the registry — and AT-8b keeps the FIELD required, because an absent list and an empty one would be two encodings of one object, which is the Z4 split. Stating the bound in the document was the alternative and was rejected: it would put a policy-authoring obligation on every deployment to serve a validator. **The direction matters more than the outcome.** The finding was reported at v1.3.31 rather than resolved by editing whichever side was cheaper, and it is resolved now by moving the side with no clause behind it — not by editing the fixture until the instrument went quiet. Found by `tools/check-attestation-object.py` on its first run, which is the check v1.3.31 shipped for a different defect (ACP-308).
>
...
- **AT-1 (REVISED in v1.3.4 — closes Y4; AMENDED in v1.3.31 — ACP-299; AMENDED in v1.3.32 — ACP-308).** ATTEST creates an **Attestation Object** binding: **`alg`** (the signature suite this attestation was raised under: a machine suite registered in the signed bundle under CR-1, or one of HM-4's two WebAuthn key types for a `webauthn` entry, which CR-8 places outside the CR-4 floor), `proposal_hash`, `policy_bundle_hash`, `bundle_epoch`, `context_snapshot_hash`, floor-only risk, required attester roles and counts, **`operator`**, `expires_at` (the issue instant plus the bundle's `attestation_window_seconds`, PB-13; ≤ 60 min, L-15), and a unique **`attestation_id`** = SHA-256 over the canonical CBOR encoding (AT-8a) of those fields plus a 128-bit fresh attestation nonce. `operator` is included so that approver-distinctness (AT-2) and the step-9 capability recheck key on a **signature-covered** value rather than a receipt-body claim; mechanized as `Y4_OperatorTamperDetected` (Annex D). **The required attester roles are a REQUIRED field that MAY be EMPTY (v1.3.32, ACP-308).** AT-8b keeps the field itself mandatory — an absent list and an empty list would be two encodings of one object, which is the Z4 split — but this document sets no lower bound on its length, and an empty list means the rule demanded a quorum and named no role, so any enrolled attester of the right kind satisfies it. It withholds nothing a verifier needs: the threshold is `quorum_k`, recomputed from the signed bundle and never read from this object (AT-9, PB-6), and role membership is resolved against the attester registry. `attestation_object.schema.json` declared `minItems: 1` through v1.3.31 and no clause did; the bound is removed rather than stated, because stating it would oblige every floor-HIGH rule an operator writes to invent a role name for a requirement it does not have.
- **AT-2.** Two kinds:
